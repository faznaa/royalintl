...
const OurExpertise: React.FC = () => {
  ...
  const rightPathLengthFirst = useTransform(scrollYProgress, [0.6, 0.8], [0, 1.2]);
  const rightPathLengthSecond = useTransform(scrollYProgress, [0.6, 0.8], [0, 1.2]);
  const rightPathLengthThird = useTransform(scrollYProgress, [0.6, 0.8], [0, 1.2]);
  const rightPathLengthFourth = useTransform(scrollYProgress, [0.6, 0.8], [0, 1.2]);
  ...
  return (
    ...
    >
      <motion.div
        className="fixed top-0 w-full h-screen flex justify-center items-center -z-10"
      >
        <GoogleGeminiEffect
          svgPaths={leftPaths}
          pathLengths={[
